...
| Test | Dataset Count (Rows) | Executed Time (ms) | User Time (ms) | System Time (ms) | Throughput (RPS) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **1** | 5,508 | 1.64 | 0.23 | 1.47 | **approx 3.3 million rows** |
| **2** | 396,647 | 13.46 | 11.05 | 2.41 | **approx 29.5 million rows** |
| **3** | 3,173,183 | 88.27 | 78.76 | 9.14 | **approx 35.9 million rows** |
| **4** | 6,346,367 | 169.73 | 138.69 | 30.48 | **approx 37.4 million rows** |
| **5** | **11,908,037** | **367.70** | **304.11** | **60.99** | **approx 32.4 million rows** |

---

## Key Findings

### 1. Consistent C-Level Throughput
...
When loading complex formats like **JSON** or connecting to **SQLite**, the overall throughput drops significantly. This is due to the necessary overhead of complex parsing state machines and database connection latency, confirming that LunivCore's speed advantage is maximized on flat, streamable data like CSV.